...
void fileManager::ExtractInfo(std::vector<std::vector<double>>& output, bool All)
{
    std::string line{ "" };
    ...
    if (All)
    {
        while (std::getline(this->Content, line))
        {
            output.push_back(VstringToVdouble(split(line, ' ')));
        }
    }
    else
    {
...
std::vector<double> fileManager::VstringToVdouble(std::vector<std::string> Vstring)
{
